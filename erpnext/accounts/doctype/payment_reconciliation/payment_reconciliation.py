...
class PaymentReconciliation(Document):
    ...
    def get_nonreconciled_payment_entries(self):
        ...
        returned_invoices = self.get_returned_invoices()

        self.add_payment_entries(payment_entries + journal_entries + returned_invoices)
    ...
    def get_returned_invoices(self):
        party_filter = "and inv.customer = '%s'" if self.party_type == "Customer" else "and inv.supplier = '%s'"
        query = """SELECT '{typ} Invoice' as reference_type, inv.name as reference_name,
                            inv.posting_date,
                            inv.outstanding_amount * -1 as amount
                            from `tab{typ} Invoice` inv
                            where inv.docstatus=1
                            and inv.outstanding_amount < 0
                            and inv.debit_to = '{account}'
                            {party_filter}
                            order by inv.posting_date
                            {lim}""".format(typ="Sales" if self.party_type == "Customer" else "Purchase",
                                            party_filter=party_filter % self.party,
                                            account=self.receivable_payable_account,
                                            lim=("limit %s" % self.limit) if self.limit else "")
        return list(frappe.db.sql(query, as_dict=1))
